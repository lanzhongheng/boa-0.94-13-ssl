...
int auth_check_userpass(char *user,char *pass,FILE *authfile)
{
  ...
  /* Encode password ('pass') using one-way function and then use base64
	...
	while (p = fgets(temps,0x100,authfile))
	{
...
int auth_authorize(request * req)
{
  ...
  LOG_D("auth_authorize\n");

  /* if web root directory authorize required, current->directory is "/" */
	hash = get_auth_hash_value("/");
  ...
  while (current) {
	  LOG_D("req_uri=%s\ncur_dir=%s, cur_dir_len=%d\n", req->request_uri,
			  current->directory, current->dir_len);
		if (!memcmp(req->request_uri, current->directory,
			...
			if (current->directory[current->dir_len - 1] != '/' &&
				...
				LOG_D("auth_authorizie break");
				break;
			}
			if (req->authorization)
			{
				LOG_D("req->authorization=%s\n", req->authorization);
				int denied = 1;
