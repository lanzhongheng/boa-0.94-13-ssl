...
#if USE_AUTH
		conn->authorization = req->authorization;
#endif
#if USE_SSL
		conn->ssl = req->ssl;
#endif
...
int process_option_line(request * req)
{
    ...
    *value++ = '\0';            /* overwrite the : */
    ...
    /* #endif */
		...
		req->authorization = (char *) malloc(2048);
		if (req->authorization) {
			strncpy(req->authorization, value, 2048);
		}
...
void free_requests(void)
{
    ...
    while (ptr != NULL) {
        next = ptr->next;
#if USE_AUTH
		if (ptr->authorization)
			free(ptr->authorization);
		ptr->authorization = NULL;
#endif
#if USE_SSL
		if (ptr->ssl)
			SSL_free(ptr->ssl);
