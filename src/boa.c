...
static void init_ssl()
{
	...
	ret = SSL_CTX_use_certificate_file(g_sslctx, 
			SSL_CERT, SSL_FILETYPE_PEM);
	if (ret <= 0) {
		DIE("SSL_CTX_use_certificate_file failed");
	}

	ret = SSL_CTX_use_PrivateKey_file(g_sslctx, SSL_KEY,
			SSL_FILETYPE_PEM);
	if (ret <= 0) {
		DIE("SSL_CTX_use_PrivateKey_file failed");
	}

	ret = SSL_CTX_check_private_key(g_sslctx);
	if (!ret) {
		DIE("SSL_CTX_check_private_key failed");
	}
	//syslog("SSL initialized");
}
#endif
